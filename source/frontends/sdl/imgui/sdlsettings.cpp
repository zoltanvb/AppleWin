...
namespace
{
    ...
    template <typename T> void drawShortcutTable(const char *label, const T &keys)
    {
...
namespace sa2
{
        ...
        if (ImGui::Begin("Settings", &myShowSettings))
        {
            ...
            if (ImGui::BeginTabBar("Settings"))
            {
                ...
                if (ImGui::BeginTabItem("Speed"))
                {
                    ...
                    /*              */ if (ImGui::Button("0.5 MHz"))
                        setSpeedMultiplier(frame, SPEED_MIN);
                    ImGui::SameLine();
                    if (ImGui::Button("1 MHz"))
                        setSpeedMultiplier(frame, SPEED_NORMAL);
                    ImGui::SameLine();
                    if (ImGui::Button("2 MHz"))
                        setSpeedMultiplier(frame, SPEED_NORMAL * 2);
                    ImGui::SameLine();
                    if (ImGui::Button("3 MHz"))
                        setSpeedMultiplier(frame, SPEED_NORMAL * 3);
                    ImGui::SameLine();
                    if (ImGui::Button("MAX"))
                        setSpeedMultiplier(frame, SPEED_MAX);
                        ...
                        if (ImGui::BeginCombo("pcap", current_interface.c_str()))
                        {
                            ...
                            if (PCapBackend::tfe_enumadapter_open())
                            {
                                ...
                                for (const auto &iface : ifaces)
                                {
                                    ...
                                    {
                                        // the following line interacts with tfe_enumadapter, so we must run it outside
                                        // the above loop
                ...
                if (ImGui::BeginTabItem("Disks", nullptr, disksFlags))
                {
                    ...
                    if (ImGui::BeginTable("Disk2", 13, ImGuiTableFlags_RowBg | ImGuiTableFlags_SizingFixedFit))
                    {
                        ...
                        for (int slot = SLOT5; slot < NUM_SLOTS; ++slot)
                        {
                            ...
                            {
                                Disk2InterfaceCard *card2 =
                                    dynamic_cast<Disk2InterfaceCard *>(cardManager.GetObj(slot));
                                ...
                                for (size_t drive = DRIVE_1; drive < NUM_DRIVES; ++drive)
                                {
                                    ...
                                    if (ImGui::RadioButton(
                                            "##Sel", (dragAndDropSlot == slot) && (dragAndDropDrive == drive)))
                                    {
                            ...
                            {
                                HarddiskInterfaceCard *pHarddiskCard =
                        ...
                        for (size_t slot = SLOT1; slot < NUM_SLOTS; ++slot)
                        {
                            ...
                            if (card == CT_Uthernet || card == CT_Uthernet2)
                            {
                                ...
                                if (ImGui::BeginTabItem(slotNumber.c_str()))
                                {
                                    ...
                                    ImGui::Separator();
                                    const NetworkCard &networkCard =
                                        dynamic_cast<NetworkCard &>(cardManager.GetRef(slot));
                                    const std::shared_ptr<NetworkBackend> &backend = networkCard.GetNetworkBackend();
                                    const std::shared_ptr<SlirpBackend> slirp =
                                        std::dynamic_pointer_cast<SlirpBackend>(backend);
                                    if (slirp)
                                    {
                    ...
                    if (ImGui::BeginTable("Registry", 2, ImGuiTableFlags_RowBg | ImGuiTableFlags_SizingFixedFit))
                    {
                        ...
                        if (open)
                        {
                            const std::map<std::string, std::map<std::string, std::string>> values =
                                Registry::instance->getAllValues();
                            ...
                            for (const auto &it1 : values)
                            {
                                ...
                                if (ImGui::TreeNodeEx(it1.first.c_str(), ImGuiTreeNodeFlags_SpanFullWidth))
                                {
                                    ...
                                    for (const auto &it2 : it1.second)
                                    {
                                        ...
                                        ImGui::TableNextColumn();
                                        ImGui::TreeNodeEx(
                                            it2.first.c_str(), ImGuiTreeNodeFlags_Leaf | ImGuiTreeNodeFlags_Bullet |
    ...
    void ImGuiSettings::openDiskFileDialog(
